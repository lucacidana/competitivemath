...
let skipValue = new URLSearchParams(window.location.search).get('skip')

fetch(
  `/users/${window.location.href.split('/')[4]}/solutions/data` +
  ...
  response
    ...
    .then((data) => {
      ...
      data.forEach((solution) => {
        ...
        document.querySelector(`#category${i}`).textContent = solution.category
